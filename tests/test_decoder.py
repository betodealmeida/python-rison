...
class TestDecoder(unittest.TestCase):
    ...
    def test_dict(self) -> None:
        ...
        self.assertEqual(rison.loads("(a:0,b:1)"), {"a": 0, "b": 1})
        self.assertEqual(
            rison.loads("(a:0,b:foo,c:'23skidoo')"),
            {"a": 0, "c": "23skidoo", "b": "foo"},
        )
        self.assertEqual(
            rison.loads("(id:!n,type:/common/document)"),
            {"type": "/common/document", "id": None},
        )
        self.assertEqual(rison.loads("(a:0)"), {"a": 0})
        self.assertEqual(rison.loads("(a:%)"), {"a": "%"})
        self.assertEqual(rison.loads("(a:/w+/)"), {"a": "/w+/"})
    ...
    def test_invalid(self) -> None:
        self.assertRaisesRegex(
            rison.decoder.ParserException, r"unmatched '\('", rison.loads, "("
        )
        self.assertRaisesRegex(
            rison.decoder.ParserException, r"unmatched '\('", rison.loads, "(a:("
        )
    ...
    def test_number(self) -> None:
        ...
        self.assertEqual(rison.loads("1e30"), 1e30)
    ...
    def test_invalid_format(self) -> None:
        with self.assertRaises(ValueError):
            rison.loads("a", format=cast(Any, int))
    ...
    def test_unmatched_array(self) -> None:
        with self.assertRaises(rison.decoder.ParserException):
            rison.loads("!(1,2")
    ...
    def test_array_extra_comma(self) -> None:
        with self.assertRaises(rison.decoder.ParserException):
            rison.loads("!(,1)")

    def test_unknown_bang_literal(self) -> None:
        with self.assertRaises(rison.decoder.ParserException):
            rison.loads("!x")

    def test_bang_at_end_of_input(self) -> None:
        with self.assertRaises(rison.decoder.ParserException):
            rison.loads("!")

    def test_dict_missing_comma(self) -> None:
        with self.assertRaises(rison.decoder.ParserException):
            rison.loads("(a:0 b:1)")

    def test_dict_extra_comma(self) -> None:
        with self.assertRaises(rison.decoder.ParserException):
            rison.loads("(,a:0)")

    def test_dict_missing_colon(self) -> None:
        with self.assertRaises(rison.decoder.ParserException):
            rison.loads("(a 0)")

    def test_unmatched_single_quote(self) -> None:
        with self.assertRaises(rison.decoder.ParserException):
            rison.loads("'abc")

    def test_invalid_string_escape(self) -> None:
        with self.assertRaises(rison.decoder.ParserException):
            rison.loads("'a!b'")
    ...
    def test_number_with_frac_and_exp(self) -> None:
