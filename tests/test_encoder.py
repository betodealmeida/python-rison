...
class TestEncoder(unittest.TestCase):
    ...
    def test_dict(self) -> None:
        ...
        self.assertEqual("(a:0,b:1)", rison.dumps({"a": 0, "b": 1}))
        self.assertEqual(
            "(a:0,b:foo,c:'23skidoo')",
            rison.dumps({"a": 0, "c": "23skidoo", "b": "foo"}),
        )
        self.assertEqual(
            "(id:!n,type:/common/document)",
            rison.dumps({"type": "/common/document", "id": None}),
        )
        self.assertEqual("(a:0)", rison.dumps({"a": 0}))
        self.assertEqual("(a:%)", rison.dumps({"a": "%"}))
        self.assertEqual("(a:/w+/)", rison.dumps({"a": "/w+/"}))
    ...
    def test_number(self) -> None:
        ...
        self.assertEqual("1e30", rison.dumps(1e30))
    ...
    def test_encode_array(self) -> None:
        ...
        with self.assertRaises(AssertionError):
            encode_array(cast(list[Any], "not a list"))
    ...
    def test_encode_object(self) -> None:
        ...
        with self.assertRaises(AssertionError):
            encode_object(cast(dict[str, Any], "not a dict"))
    ...
    def test_dumps_o_rison(self) -> None:
        ...
        with self.assertRaises(ValueError):
            rison.dumps([1, 2], format=cast(Any, "O"))

    def test_dumps_invalid_format(self) -> None:
        with self.assertRaises(ValueError):
            rison.dumps("x", format=cast(Any, "invalid"))
